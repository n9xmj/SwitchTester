...
void v_acon_op_eventq_test(char c_op, char *pc_line)
{
    ...
    switch (c_sub)
    {
         ...
         *----------------------------------------------------------------*/
        case 'I':
            v_acon_emit(ACON_SIG_OK, "%s,I,N%X,F%lX,E%X,Z%lX,O%lX,P%lX",
                        ...
                        (unsigned long) u32_event_queue_puts(&s_x_eq_test));
            return;

        /*------------------------------------------------------------------
         * F,R[,<sel>] -- reset the diagnostic counters. sel: 0/absent both,
         * 1 drops only, 2 puts only. Selectable so the host can prove the two
         * reset-ers are independent.
         *----------------------------------------------------------------*/
        case 'R':
            if ((u32_a == 0u) || (u32_a == 1u))
            {
                v_event_queue_dropped_reset(&s_x_eq_test);
            }
            if ((u32_a == 0u) || (u32_a == 2u))
            {
                v_event_queue_puts_reset(&s_x_eq_test);
            }
            v_acon_emit(ACON_SIG_OK, "%s,R,O%lX,P%lX", pc_acon_op_name(c_op, ac_op),
                        (unsigned long) u32_event_queue_dropped(&s_x_eq_test),
                        (unsigned long) u32_event_queue_puts(&s_x_eq_test));
            return;
