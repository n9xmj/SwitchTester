...
void v_acon_op_nvm_test(char c_op, char *pc_line)
{
    ...
    switch (c_sub)
    {
        ...
        case 'I':
        {
            const nvm_header_t *p_x_hdr = (const nvm_header_t *) g_x_nvm_test.p_v_data;

            if (g_x_nvm_test.p_v_data == NULL)
            {
            ...
            v_acon_emit(ACON_SIG_OK, "%s,I,G%lX,C%lX,W%lX,D%X,Z%lX,B%X,T%lX,U%lX",
                        ...
                        g_x_nvm_test.u8_need_commit,
                        (unsigned long) g_x_nvm_test.u32_size,
                        g_x_nvm_test.u8_wear_blocks,
                        (unsigned long) g_x_nvm_test.u32_block_stride,
                        (unsigned long) g_x_nvm_test.u32_alloc_unit);
            return;
        }
        ...
        case 'L':
        {
            ...
            p_x_obj = (const nvm_object_t *)
                      ((const uint8_t *) g_x_nvm_test.p_v_data + u32_offset);

            while ((p_x_obj->x_id != NVM_PARAM_END_OF_DATA) &&
                   (u32_offset < g_x_nvm_test.u32_size))
            {
        ...
        case 'C':
        {
            ...
            x_status = x_nvm_create(&g_x_nvm_test, (nvm_param_id_t) u32_a,
        ...
        case 'G':
        {
            ...
            x_status = x_nvm_get(&g_x_nvm_test, (nvm_param_id_t) u32_a, &x_value);
        ...
        {
            nvm_test_value_t x_value = (nvm_test_value_t) u32_b;
            x_status = x_nvm_set(&g_x_nvm_test, (nvm_param_id_t) u32_a, &x_value);
            v_nvm_reply_status(c_op, c_sub, x_status);
            return;
        }
        ...
        /* N,D,<id> -- delete. */
        case 'D':
            x_status = x_nvm_delete(&g_x_nvm_test, (nvm_param_id_t) u32_a);
            v_nvm_reply_status(c_op, c_sub, x_status);
            return;

        /* N,K -- commit. NO_CHANGE is a normal, assertable outcome. */
        case 'K':
            x_status = x_nvm_commit(&g_x_nvm_test);
            v_nvm_reply_status(c_op, c_sub, x_status);
            return;
        ...
        case 'R':
            if (g_x_nvm_test.p_v_data != NULL)
            {
                ...
                g_x_nvm_test.u8_need_commit = 0;
                (void) x_nvm_pool_release(&g_x_nvm_test);
            }
            x_nvm_test_config.x_init_policy   = (nvm_init_policy_t) u32_a;
            x_nvm_test_config.ux_base_address = ux_nvm_drv_ram_base();
            x_status = x_nvm_pool_init(&g_x_nvm_test, &x_nvm_test_config);
            v_nvm_reply_status(c_op, c_sub, x_status);
            return;
        ...
        case 'A':
        {
            ...
            v_nvm_drv_ram_counts(&u32_reads, &u32_writes);
         ...
         *----------------------------------------------------------------*/
        case 'T':
            v_nvm_commit_timer_tick(&g_x_nvm_test, (uint16_t) u32_a);
            v_acon_emit(ACON_SIG_OK, "%s,T,C%X,E%X",
                        pc_acon_op_name(c_op, ac_op),
                        g_x_nvm_test.u16_commit_timer,
                        b_nvm_commit_time_elapsed(&g_x_nvm_test, (uint16_t) u32_b) ? 1 : 0);
            return;
